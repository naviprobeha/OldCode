...
namespace Navipro.Infojet.Lib
{
	...
	public class ItemAttributeValue
	{
		...
		{
            DatabaseQuery databaseQuery = database.prepare("SELECT [Item No_], [Item Attribute Code], [Value], [Language Code] FROM [" + database.getTableName("Item Attribute Value") + "] WHERE [Item No_] = @itemNo AND [Item Attribute Code] = @itemAttributeCode AND [Language Code] = @languageCode");
