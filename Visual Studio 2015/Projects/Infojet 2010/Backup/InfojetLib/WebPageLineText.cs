...
namespace Navipro.Infojet.Lib
{
	...
	public class WebPageLineText
	{
        ...
        public void update()
        {
            ...
            SqlDataReader dataReader = database.query("SELECT [Web Site Code], [Web Page Code], [Web Page Line No_], [Language Code], [Version No_], [Line No_], [Text] FROM [" + database.getTableName("Web Page Line Text") + "] WHERE [Web Site Code] = '" + this.webSiteCode + "' AND [Web Page Code] = '" + this.webPageCode + "' AND [Web Page Line No_] = '" + this.webPageLineNo + "' AND [Language Code] = '" + this.languageCode + "' AND [Version No_] = '" + this.versionNo + "' AND [Line No_] = '" + this.lineNo + "'");

            if (dataReader.Read())
            {
                dataReader.Close();

                database.nonQuery("UPDATE [" + database.getTableName("Web Page Line Text") + "] SET [Text] = '" + text + "' WHERE [Web Site Code] = '" + this.webSiteCode + "' AND [Web Page Code] = '" + this.webPageCode + "' AND [Web Page Line No_] = '" + this.webPageLineNo + "' AND [Language Code] = '" + this.languageCode + "' AND [Version No_] = '" + this.versionNo + "' AND [Line No_] = '" + this.lineNo + "'");

            }
            else
            {
                dataReader.Close();
                database.nonQuery("INSERT INTO [" + database.getTableName("Web Page Line Text") + "] ([Web Site Code], [Web Page Code], [Web Page Line No_], [Language Code], [Version No_], [Line No_], [Text]) VALUES ('" + this.webSiteCode + "','" + this.webPageCode + "','" + this.webPageLineNo + "','" + this.languageCode + "','" + this.versionNo + "','" + this.lineNo + "')");
            }
        }
