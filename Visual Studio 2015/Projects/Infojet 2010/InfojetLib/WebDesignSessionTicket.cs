...
namespace Navipro.Infojet.Lib
{
    ...
    public class WebDesignSessionTicket : ServiceArgument
    {
        ...
        public WebDesignSessionTicket(Infojet infojetContext, string webSiteCode, string ticketId)
        {
            //
        ...
        {
            SqlDataReader dataReader = infojetContext.systemDatabase.query("SELECT [Web Site Code], [Ticket ID], [Session ID], [Created Date], [Created Time] FROM [" + infojetContext.systemDatabase.getTableName("Web Design Session Ticket") + "] WHERE [Web Site Code] = '" + this.webSiteCode + "' AND [Ticket ID] = '" + ticketId + "'");
            if (dataReader.Read())
            {
