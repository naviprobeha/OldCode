...
namespace Konvex.SmartShipping.DataObjects
{
    public class ItemPriceExtended
    {
        ...
        public static ItemPriceExtended fromJsonObject(JObject jsonObject)
        {
            ItemPriceExtended entry = new ItemPriceExtended();
            entry.entryNo = (int)jsonObject["entryNo"];
            entry.itemNo = (string)jsonObject["itemNo"];

            entry.startingDate = DateTime.Parse("2001-01-01");
            entry.endingDate = DateTime.Parse("2001-01-01");

            if (jsonObject["startingDate"] != null)
            {
            ...
            entry.customerPriceGroup = (string)jsonObject["customerPriceGroup"];
            entry.unitOfMeasureCode = (string)jsonObject["unitOfMeasureCode"];
            entry.quantityFrom = (decimal)jsonObject["quantityFrom"];
            entry.quantityTo = (decimal)jsonObject["quantityTo"];
            entry.lineAmount = (decimal)jsonObject["price"];
