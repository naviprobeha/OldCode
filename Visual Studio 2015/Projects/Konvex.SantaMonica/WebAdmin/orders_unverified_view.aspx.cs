...
namespace Navipro.SantaMonica.WebAdmin
{
	...
	/// </summary>
	public class orders_unverified_view : System.Web.UI.Page
	{
		...
		protected ArrayList customerList;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (Request["command"] == "setCustomerNo")
			{
				Customer customer = customers.getEntry(database, Request["organizationNo"], Request["customerNo"]);
				
				string directionComment = currentShipOrder.directionComment;
				string directionComment2 = currentShipOrder.directionComment2;

				currentShipOrder.customerNo = customer.no;
				currentShipOrder.applyBillToCustomer(customer);
				currentShipOrder.shipDate = DateTime.Today;
				
				currentShipOrder.directionComment = directionComment;
				currentShipOrder.directionComment2 = directionComment2;

				if ((customer.directionComment != "") && (customer.directionComment.Length > 5))
				{
					currentShipOrder.directionComment = customer.directionComment;
					currentShipOrder.directionComment2 = customer.directionComment2;
				}

				if (currentShipOrder.paymentType == 1)
				{
					currentShipOrder.comments = currentShipOrder.comments.Replace("Internet>", "Internet> Kontant betalning.");
				}

				currentShipOrder.save(database, true);

				currentShipOrder.changeOrganization(database, customer.organizationNo, currentOperator.userId);

				currentShipOrder.updatePrices(database);

				Response.Redirect("orders_unverified.aspx");
			}
			...
			{
				customerList = customers.matchCallCenterCustomersToShipOrder(database, currentShipOrder);
			}
			else
			{
				customerList = customers.matchOrganizationCustomersToShipOrder(database, currentShipOrder);
			}
		}
