...
namespace Navipro.SantaMonica.WebAdmin
{
	...
	/// </summary>
	public class customers_orders : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (Request["ongoing"] == "1")
			{
				activeShipOrders = shipOrders.getOngoingCustomerShipOrderDataSet(database, Request["organizationNo"], Request["agent"], Request["customerNo"]);
			}
			else
			{
				activeShipOrders = shipOrders.getCustomerShipOrderDataSet(database, Request["organizationNo"], Request["agent"], Request["customerNo"]);
			}


			agentsClass = new Agents();
			activeAgents = agentsClass.getDataSet(database, Request["organizationNo"], Agents.TYPE_SINGLE);
