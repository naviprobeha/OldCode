<%@ Page language="c#" Codebehind="lyncLog.aspx.cs" AutoEventWireup="false" Inherits="WebAdmin.lyncLog" %>
	...
	<body MS_POSITIONING="GridLayout" topmargin="10" onload="setTimeout('document.thisform.submit()', 60000)">
		...
		<input type="hidden" name="command" value="">
			<table cellspacing="2" cellpadding="2" border="0" width="100%" class="frame">
				...
				<tr>
					<td class="" width="95%">Ringgrupp:&nbsp;<select name="responseGroup" class="Textfield" onchange="document.thisform.submit()">
					<%
						int a = 0;
						while (a < responseGroupDataSet.Tables[0].Rows.Count)
						{
							Navipro.SantaMonica.Common.ResponseGroup responseGroup = new Navipro.SantaMonica.Common.ResponseGroup(responseGroupDataSet.Tables[0].Rows[a]);
							
							%><option value="<%= responseGroup.uri %>" <% if (currentResponseGroup.uri == responseGroup.uri) Response.Write("selected"); %>><%= responseGroup.description %></option><%
					...
					</select>&nbsp;<input type="button" value="Idag" onclick="gotoToday()" class="Button">&nbsp;Urval: <select name="filter" class="Textfield" onchange="document.thisform.submit()">
						<option value="0" <% if (Request["filter"] == "0") Response.Write("selected"); %>>20 senaste samtalen</option>
						<option value="9" <% if (Request["filter"] == "9") Response.Write("selected"); %>>Alla samtal</option>
					</select></td>
				</tr>
				...
				<tr>
					<td>
						<table cellspacing="1" cellpadding="2" border="0" width="100%" class="innerFrame">
							...
							<%
								int i = 0;
								while (i < queuedPhoneLogList.Count)
								{
									Navipro.SantaMonica.Common.ResponseGroupCall phoneLogEntry = (Navipro.SantaMonica.Common.ResponseGroupCall)queuedPhoneLogList[i];
								...
								if (i == 0)
								{
									%>
										<tr>
											<td class="jobDescription" colspan="6">Inga samtal på kö.</td>
										</tr>
									<%
								}
				...
				<tr>
					<td>
						<table cellspacing="1" cellpadding="2" border="0" width="100%" class="innerFrame">
							...
							<%
								i = 0;
								while (i < activePhoneLogList.Count)
								{
									Navipro.SantaMonica.Common.ResponseGroupCall phoneLogEntry = (Navipro.SantaMonica.Common.ResponseGroupCall)activePhoneLogList[i];
									...
									if (phoneLogEntry.pickedUpDateTime.Year > 1753) pickedUpDateTime = phoneLogEntry.pickedUpDateTime.ToString("HH:mm:ss");
									if (phoneLogEntry.finishedDateTime.Year > 1753) finishedDateTime = phoneLogEntry.finishedDateTime.ToString("HH:mm:ss");
									...
									<tr>
										...
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><%= phoneLogEntry.remoteNumber %></td>
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><%= phoneLogEntry.getStatusText() %></td>
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><% Response.Write(pickedUpDateTime); %>&nbsp;</td>
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><%= phoneLogEntry.operatorName %></td>
