...
namespace Navipro.SantaMonica.WebAdmin
{
	...
	/// </summary>
	public class orders_move : System.Web.UI.Page
	{
			...
			if (Request["command"] == "move")
			{
				if (Request["shipDateYear"] != null)
				{
					...
					catch(Exception g)
					{
						try
						{
							shipDate = new DateTime(int.Parse(Request["shipDateYear"]), int.Parse(Request["shipDateMonth"])+1, 1);
						}
						catch(Exception f)
						{
							shipDate = DateTime.Now;
				...
				while (k < activeShipOrders.Tables[0].Rows.Count)
				{
					if (Request["order"+activeShipOrders.Tables[0].Rows[k].ItemArray.GetValue(1).ToString()] == "on")
					{
						...
						currentShipOrder.shipDate = shipDate;
