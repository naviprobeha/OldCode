<%@ Page language="c#" Codebehind="status_unScaledContainers.aspx.cs" AutoEventWireup="false" Inherits="WebAdmin.status_unScaledContainers" %>
...
<HTML>
	...
	function changeShipDate()
	{
		...
		document.thisform.action = "status_unScaledContainers.aspx";
	...
	<body MS_POSITIONING="GridLayout" topmargin="10" onload="setTimeout('document.thisform.submit()', 60000)">
		...
		<input type="hidden" name="command" value="">
			<table cellspacing="2" cellpadding="2" border="0" width="100%" class="frame">
				...
				<tr>
					<td class="activityName">Containers på golvet</td>
				</tr>
				<tr>
						...
						<table cellspacing="1" cellpadding="2" border="0" width="100%" class="innerFrame">
							...
							<%
								...
								while (i < containerDataSet.Tables[0].Rows.Count)
								{
									...
									string shippingCustomerNo = containerDataSet.Tables[0].Rows[i].ItemArray.GetValue(9).ToString();
									string lineJournalEntryNo = containerDataSet.Tables[0].Rows[i].ItemArray.GetValue(10).ToString();
									DateTime entryDate = DateTime.Parse(containerDataSet.Tables[0].Rows[i].ItemArray.GetValue(11).ToString());
									DateTime entryTime = DateTime.Parse(containerDataSet.Tables[0].Rows[i].ItemArray.GetValue(12).ToString());
									
									%>
									<tr>
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><%= lineOrderContainer.lineOrderEntryNo.ToString() %></td>
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><%= entryDate.ToString("yyyy-MM-dd")+" "+entryTime.ToString("HH:mm:ss") %></td>
										...
										<td class="jobDescription" valign="top" style="<%= bgStyle %>"><a href="shippingCustomers_view.aspx?shippingCustomerNo=<%= shippingCustomerNo %>"><img src="images/button_assist.gif" border="0" width="12" height="13"></a>&nbsp;<%= shippingCustomerNo %>&nbsp;</td>										
