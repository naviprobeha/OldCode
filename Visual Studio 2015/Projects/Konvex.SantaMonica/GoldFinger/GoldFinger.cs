...
{
    public class Goldfinger : IGoldfinger, Logger
    {
        ...
        public int reportStatus(string agentCode, int rt90x, int rt90y, float heading, float speed, float height, int status, string userName, int tripMeter)
        {
            ...
            try
            {
                Goldfinger2022 newPlatform = new Goldfinger2022();
                newPlatform.init(configuration, database);
                newPlatform.reportStatus(agentCode, rt90x, rt90y, heading, speed, height, status, userName, tripMeter);
            }
            catch (Exception) { }
        ...
        public void createContainerEntry(string agentCode, string containerNo, int type, int documentType, string documentNo, DateTime entryDateTime, DateTime estimatedArrivalDateTime, int locationType, string locationCode, int positionX, int positionY, int sourceType, string sourceCode, float weight)
        {
            ...
            try
            {
                ...
                {
                    OrganizationLocations organizationLocations = new OrganizationLocations();
                    OrganizationLocation orgLoc = organizationLocations.getEntry(database, agent.organizationNo, locationCode);
                    if (orgLoc == null)
                    {
                        if (type == 2)
                        {
                ...
                if (containerEntry.type == 1 && containerEntry.locationType == 2 && containerEntry.documentNo != "")
                {
                    ...
                    LineJournal lineJournal = lineJournals.getEntry(database, containerEntry.documentNo);
                    if (lineJournal != null)
                    {
                        containerEntry.locationCode = lineJournal.arrivalFactoryCode;
                        containerEntry.save(database);
                    }
                }
            }
        ...
        public SynchEntryCollection getSynchRecord(string agentCode)
        {
            ...
            this.checkAssignedShipOrders(agentCode);
            ...
            try
            {
                ...
                serverLogging.log(agentCode, "[GetSyncRecords] Records: " + synchEntryCollection.Count);
            }
            catch (Exception) { }
            ...
            try
            {
                ...
                foreach (int synchEntryNo in synchEntryNoList)
                {
                    ...
                    if (entry != null)
                    {
                         ...
                         * */
                         /*
                        if (entry.type == SynchronizationQueueEntries.SYNC_SHIP_ORDER)
                        {
                            try
                            {
                                ShipOrders shipOrders = new ShipOrders();
                                ShipOrder shipOrder = shipOrders.getEntry(database, entry.primaryKey);
                                shipOrders.setStatus(this.database, entry.primaryKey, 4);
                            }
                            catch (Exception) { }
                        }
                        */
        ...
        public void setLineOrderStatus(string agentCode, string no, int status, int positionX, int positionY, DateTime shipTime, int loadWaitTime, string loadWaitReason)
        {
            ...
            try
            {
                serverLogging.log(agentCode, "SetLineOrderStatus: LoadWaitTime: "+loadWaitTime+", Reason: "+loadWaitReason);
                LineOrders lineOrders = new LineOrders();
                ...
                if (entry != null)
                {
                    ...
                    if (journal != null && journal.agentCode == agentCode)
                    {
                        ...
                        if (status == 7)
                        {
                            ...
                            entry.loadWaitReason = loadWaitReason;
        ...
        public void reportLineJournal(string agentCode, Konvex.SmartShipping.DataObjects.LineJournal lineJournal)
        {
            ...
            try
            {
                if (lineJournal != null)
                {
                    ...
                    LineJournal lineJournalEntry = lineJournals.getEntry(database, lineJournal.entryNo.ToString());
                    if (lineJournalEntry == null) return;
        ...
        public void createShipment(string agentCode, Konvex.SmartShipping.DataObjects.ShipmentHeader fromShipmentHeader)
        {
            ...
            try
            {
                ...
                serverLogging2.log(agentCode, "CreateShipment: 1");
                ...
                serverLogging2.log(agentCode, "CreateShipment: 2");

                shipmentHeader.save(this.database);

                serverLogging2.log(agentCode, "CreateShipment: 3");
                ...
                while (i < fromShipmentHeader.shipmentLineCollection.Count)
                {
                    ...
                    while (j < fromShipmentHeader.shipmentLineCollection[i].shipmentLineIdCollection.Count)
                    {
                        ...
                        shipmentLineId.unitId = fromShipmentHeader.shipmentLineCollection[i].shipmentLineIdCollection[j].unitId.Replace("'", "");
                        shipmentLineId.type = fromShipmentHeader.shipmentLineCollection[i].shipmentLineIdCollection[j].type;
                        shipmentLineId.reMarkUnitId = fromShipmentHeader.shipmentLineCollection[i].shipmentLineIdCollection[j].reMarkUnitId.Replace("'", "");
                ...
                serverLogging2.log(agentCode, "CreateShipment: 4");
                ...
                serverLogging2.log(agentCode, "CreateShipment: 5");
            ...
            catch (Exception ex)
            {
                ...
                StreamWriter writer = new StreamWriter("C:\\temp\\agents\\" + agentCode + "_ERROR.txt", true);
        ...
        private void checkAssignedShipOrders(string agentCode)
        {
            return;
            //Safety
            if (agentCode != "LRF") return;

            Agents agents = new Agents();
            Navipro.SantaMonica.Common.Agent agent = agents.getAgent(this.database, agentCode);

            ShipOrders shipOrders = new ShipOrders();
            DataSet dataSet = shipOrders.getActiveDataSet(database, agent.organizationNo, agentCode, DateTime.Today.AddDays(-1), DateTime.Today);
            ...
            {
                if (dataSet.Tables[0].Rows[i].ItemArray.GetValue(8).ToString() == "3")
                {
                    SqlDataAdapter dataAdapter = database.dataAdapterQuery("SELECT * FROM [Synchronization Queue] WHERE [Agent Code] = '" + agentCode.Replace("'", "") + "' AND [Type] = 0 AND [Primary Key] = '" + dataSet.Tables[0].Rows[i].ItemArray.GetValue(1).ToString() + "'");
                    DataSet queueDataSet = new DataSet();
                    dataAdapter.Fill(queueDataSet);

                    if (queueDataSet.Tables[0].Rows.Count == 0)
                    {
                        SynchronizationQueueEntries syncEntries = new SynchronizationQueueEntries();
                        syncEntries.enqueue(database, agentCode, 0, dataSet.Tables[0].Rows[i].ItemArray.GetValue(1).ToString(), 0);
        ...
        private void directUnloadOfLineOrder(ContainerEntry containerEntry)
        {
            ...
            if (containerEntry.sourceType == 0)
            {
                ...
                {
                    Agents agents = new Agents();
                    Agent agent = agents.getAgent(this.database, containerEntry.sourceCode);
                    ...
                    if (lineJournal != null)
                    {
                        ...
                        lineJournal.organizationNo = agent.organizationNo;
                        lineJournal.save(this.database);
                        containerLineOrder.organizationNo = agent.organizationNo;
                        containerLineOrder.save(this.database);
        ...
        public Konvex.SmartShipping.DataObjects.LineOrderCollection getLineOrders(string agentCode, int lineJournalNo, DateTime date)
        {
            DateTime fromDate = DateTime.Today.AddDays(-3);
            DateTime toDate = DateTime.Today.AddDays(10);
            ...
            LineOrders lineOrders = new LineOrders();
            DataSet dataSet = lineOrders.getActiveDataSet(database, agentCode, fromDate, toDate);
            ...
            while (i < dataSet.Tables[0].Rows.Count)
            {
                ...
                if (lineJournalNo > 0)
                {
                    if (lineOrderObject.lineJournalEntryNo == lineJournalNo) lineOrderCollection.Add(lineOrderObject);
                }
                else
                {
        ...
        public void setLineOrderContainers(string agentCode, string no, List<string> containerStringList)
        {
            ...
            try
            {
                ...
                string originalComment = entry.comments;
                ...
                if (journal != null && journal.agentCode == agentCode)
                {
                    ...
                    {
                        //Containers som ligger på annan linjeorder...

                        serverLogging.log(agentCode, "Checking container to move " + containerNo + ".");
                        ...
                        {
                            serverLogging.log(agentCode, "Moving container: " + containerLineOrder.entryNo+" to current order.");
                            ...
                            int currentJournalEntryNo = entry.lineJournalEntryNo;
                            
                            //Nytt från 2019-07-23
                            //containerLineOrder.lineJournalEntryNo = currentJournalEntryNo;
                            //containerLineOrder.status = 4;
                            //containerLineOrder.save(database);
                            //
                            ...
                            entry.comments = containerLineOrder.comments;
                            entry.save(database);


                            containerLineOrder.comments = "";
                            containerLineOrder.save(database);
                            containerLineOrder.updateDetails(database);

                            if (containerLineOrder.countContainers(database) == 0) containerLineOrder.delete(database);
                        }
                    ...
                    while (j < containerDataSet.Tables[0].Rows.Count)
                    {
                        if (!containerStringList.Contains(containerDataSet.Tables[0].Rows[j].ItemArray.GetValue(2).ToString()))
                        {
                            ...
                            newLineOrder.comments = originalComment;
                            newLineOrder.shippingCustomerName = entry.shippingCustomerName;
                            newLineOrder.address = entry.address;
                            newLineOrder.address2 = entry.address2;
                            newLineOrder.postCode = entry.postCode;
                            newLineOrder.city = entry.city;
                            newLineOrder.countryCode = entry.countryCode;
                            ...
                            newLineOrder.updateDetails(database);

                            newLineOrder.enableAutoPlan = true;
                            newLineOrder.confirmedToDateTime = DateTime.Now;
                            newLineOrder.shipTime = new DateTime(1974, 1, 1, DateTime.Now.Hour, 0, 0);
                            newLineOrder.save(database);
                        }
                        j++;
                    }
