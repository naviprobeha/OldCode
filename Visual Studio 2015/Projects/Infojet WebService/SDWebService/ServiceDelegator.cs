...
namespace Navipro.Infojet.WebService
{
	...
	public class ServiceDelegator
	{
        ...
        public XmlDocument transport(XmlDocument xmlDocument)
        {
            ...
            Navipro.Infojet.Lib.DatabaseQuery databaseQuery = database.prepare("INSERT INTO [" + database.getTableName("SD Request Queue") + "] ([Guid], [Inbound Document], [Outbound Document], [Created DateTime], [Processed DateTime], [Processed]) VALUES ('" + guid.ToString() + "', '" + xmlDocument.OuterXml + "', '', GETDATE(), '1754-01-01', 0)");
            databaseQuery.execute();


            net.workanywhere.sirius.SDRequestWebService serviceRequest = new Navipro.Infojet.WebService.net.workanywhere.sirius.SDRequestWebService();
            ...
            databaseQuery = database.prepare("SELECT [Outbound Document] FROM [" + database.getTableName("SD Request Queue") + "] WHERE [Guid] = '" + guid.ToString() + "'");
