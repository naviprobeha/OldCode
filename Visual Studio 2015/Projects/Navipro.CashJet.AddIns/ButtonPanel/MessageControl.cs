...
namespace Navipro.CashJet.AddIns
{
    public class MessageControl
    {
        ...
        public int showInputMaxLength(string title, string message, string button1, string button2, int maxLength, ref string inputText)
        {
            InputBox inputBox = new InputBox();
            inputBox.init(title, message, button1, button2, maxLength);
            inputBox.ShowDialog();
            int result = inputBox.getResult();
            inputText = inputBox.getInputText();
            inputBox.Dispose();
