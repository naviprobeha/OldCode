...
namespace Navipro.SmartInventory
{
    ...
    public class DataPickHeader : ServiceArgument
    {
        ...
        public void save()
        {
            ...
            try
            {
                ...
                if (!dataReader.Read())
                {
                    ...
                    //smartDatabase.nonQuery("INSERT INTO pickHeader (no, assignedTo, noOfLines, description) VALUES ('" + _no + "', '" + _assignedTo + "', '" + _noOfLines + "', '" + _description + "')");
                }
                else
                {
                    smartDatabase.nonQuery("UPDATE pickHeader SET assignedTo = '" + _assignedTo + "', noOfLines = '" + _noOfLines + "' WHERE no = '" + _no + "'");
                    //smartDatabase.nonQuery("UPDATE pickHeader SET assignedTo = '" + _assignedTo + "', noOfLines = '" + _noOfLines + "', description = '" + _description + "' WHERE no = '" + _no + "'");
                }
                dataReader.Close();
                dataReader.Dispose();
        ...
        private void fromDataReader(SqlCeDataReader dataReader)
        {
            ...
            this._noOfLines = dataReader.GetInt32(1);
            this._assignedTo = (string)dataReader.GetValue(2);
            /*            
            this._no = (string)dataReader.GetValue(0);            
            this._noOfLines = dataReader.GetInt32(1);
            this._description = (string)dataReader.GetValue(2);
            this._assignedTo = (string)dataReader.GetValue(3);
            */
        }
        ...
        private void fromDataRow(DataRow dataRow)
        {
            ...
            this._noOfLines = int.Parse(dataRow.ItemArray.GetValue(1).ToString());
            this._assignedTo = dataRow.ItemArray.GetValue(2).ToString();
            /*
            this._no = dataRow.ItemArray.GetValue(0).ToString();            
            this._noOfLines = int.Parse(dataRow.ItemArray.GetValue(1).ToString());
            this._description = dataRow.ItemArray.GetValue(2).ToString();
            this._assignedTo = dataRow.ItemArray.GetValue(3).ToString();
            */
        }
        ...
        public static void fromDOM(XmlElement activitiesElement, SmartDatabase smartDatabase)
        {
            ...
            while (i < activityNodeList.Count)
            {
                ...
                //string descripton = activitiesElement.SelectSingleNode("description").FirstChild.Value;
                ...
                //dataWhsePickHeader.description = descripton;
        ...
        public static DataSet getDataSet(SmartDatabase smartDatabase)
        {
            ...
            //SqlCeDataAdapter adapter = smartDatabase.dataAdapterQuery("SELECT no, assignedTo, noOfLines, description FROM pickHeader ORDER BY no");
        ...
        public static DataPickHeader getPickHeader(SmartDatabase smartDatabase, string documentNo)
        {
            ...
            //SqlCeDataReader dataReader = smartDatabase.query("SELECT no, assignedTo, noOfLines, description FROM pickHeader WHERE no = '" + documentNo + "'");
        ...
        public XmlElement toDOM(XmlDocument xmlDocumentContext)
        {
            ...
            while (i < dataSet.Tables[0].Rows.Count)
            {
                ...
                if (dataPickLine.picked == true)
                {
                    ...
                    lineElement.SetAttribute("binCode", dataPickLine.binCode.ToString()); //AKMA
