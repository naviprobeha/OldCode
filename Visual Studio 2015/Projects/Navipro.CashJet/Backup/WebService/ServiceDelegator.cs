...
namespace Navipro.CashJet.WebService
{
    ...
    public class ServiceDelegator
    {
        ...
        public XmlDocument transport(XmlDocument xmlDocument)
        {
            ...
            if (performLocalTasks(xmlDocument, out responseDocument))
            {
        ...
        private bool performLocalTasks(XmlDocument xmlDocument, out XmlDocument responseDocument)
        {
            responseDocument = new XmlDocument();
 
 
            XmlElement serviceNameElement = (XmlElement)xmlDocument.SelectSingleNode("nav/serviceRequest/serviceName");
            if (serviceNameElement != null)
            {
                if (serviceNameElement.FirstChild.Value == "getInventory")
                {
                    InventoryService inventoryService = new InventoryService(configuration);
                    responseDocument = inventoryService.performService(xmlDocument);

                    return true;
                }

                if (serviceNameElement.FirstChild.Value == "getReceiptReturnInfo")
                {
                    ReturnReceiptService returnReceiptService = new ReturnReceiptService(configuration);
                    responseDocument = returnReceiptService.performService(xmlDocument);

                    return true;
                }

                if (serviceNameElement.FirstChild.Value == "getOrderReturnInfo")
                {
                    ReturnOrderService returnOrderService = new ReturnOrderService(configuration);
                    responseDocument = returnOrderService.performService(xmlDocument);
