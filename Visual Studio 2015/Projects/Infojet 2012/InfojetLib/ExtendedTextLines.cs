...
namespace Navipro.Infojet.Lib
{
	...
	public class ExtendedTextLines
	{
        ...
        public DataSet getTextLines(int tableName, int textNo, string languageCode)
        {
            DatabaseQuery databaseQuery = database.prepare("SELECT [Table Name], [No_], [Language Code], [Text No_], [Line No_], [Text] FROM [" + database.getTableName("Extended Text Line") + "] WHERE [Table Name] = @tableName AND [Language Code] = @languageCode AND [Text No_] = @textNo ORDER BY [Line No_]");
