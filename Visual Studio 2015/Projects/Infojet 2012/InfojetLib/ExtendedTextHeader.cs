...
namespace Navipro.Infojet.Lib
{
	...
	public class ExtendedTextHeader
	{
		...
		{
            DatabaseQuery databaseQuery = database.prepare("SELECT [Table Name], [No_], [Language Code], [Text No_], [Starting Date], [Ending Date], [All Language Codes] FROM [" + database.getTableName("Extended Text Header") + "] WHERE [Table Name] = @tableName AND [No_] = @no AND [Language Code] = @languageCode AND [Text No_] = @textNo AND [Publish On Web] = 1");
        ...
        public static Hashtable getItemTexts(Infojet infojetContext, string languageCode, bool fullText)
        {
            Hashtable hashTable = new Hashtable();

            ExtendedTextLines extTextLines = new ExtendedTextLines(infojetContext.systemDatabase);
            DataSet dataSet = extTextLines.getTextLines(2, 1, languageCode);

            int i = 0;
            while (i < dataSet.Tables[0].Rows.Count)
            {
                ExtendedTextLine extendedTextLine = new ExtendedTextLine(infojetContext.systemDatabase, dataSet.Tables[0].Rows[i]);

                if ((extendedTextLine.lineNo <= 30000) || (fullText))
                {
                    string text = "";
                    if (hashTable.Contains(extendedTextLine.no)) text = (string)hashTable[extendedTextLine.no];
                    text = text + " " + extendedTextLine.text;

                    if (!hashTable.Contains(extendedTextLine.no))
                    {
                        hashTable.Add(extendedTextLine.no, text);
                    }
                    else
                    {
                        hashTable[extendedTextLine.no] = text;
                    }
                }
                i++;
            }
