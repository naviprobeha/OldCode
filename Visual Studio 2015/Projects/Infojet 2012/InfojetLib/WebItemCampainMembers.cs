...
namespace Navipro.Infojet.Lib
{
	...
	public class WebItemCampainMembers
	{
		...
		{
            DatabaseQuery databaseQuery = infojetContext.systemDatabase.prepare("SELECT [No_], [Type], [No_] as Code, [Item Info Web Page Code] FROM [" + infojetContext.systemDatabase.getTableName("Web Item Campain Member") + "] WHERE [Web Site Code] = @webSiteCode AND [Web Item Campain Code] = @webItemCampainCode ORDER BY [Sort Order], [No_]");
