...
namespace Navipro.Infojet.Lib
{
    public class EditorManagerControl
    {
        ...
        {
            Link refreshLink = infojetContext.webPage.getUrlLink();
            Link editLink = infojetContext.webPage.getUrlLink();
            editLink.addParameter("contentCommand", "edit");
            editLink.addParameter("lineNo", webPageLine.lineNo.ToString());
            
            Link closeLink = infojetContext.webPage.getUrlLink();
            closeLink.addParameter("contentCommand", "closeDesignMode");
            ...
            output.WriteLine(" document.location.href = '" + closeLink.toUrl() + "';");
            output.WriteLine(" }");

            output.WriteLine(" function openEditWindow" + this.controlId + "() { window.open(\"" + editLink.toUrl() + "\",\"Redigera\",\"left=100, top=30, width=800, height=800, toolbar=no, location=no, scrollbars=yes, resizable=yes\"); } ");

            output.WriteLine("</script>");

            output.WriteLine("<div id=\"content_" + this.controlId + "\">");
            output.WriteLine("<div style=\"float: right; align: right;\"><a href=\"#\" onclick=\"closeDesignMode" + this.controlId + "()\" style=\"padding: 1px;\"><img src=\"" + infojetContext.webSite.siteLocation + "_assets/img/cms_close.gif\" alt=\"Stäng\"/></a><a href=\"#\" onclick=\"openEditWindow" + this.controlId + "()\" style=\"padding: 1px;\"><img src=\"" + infojetContext.webSite.siteLocation + "_assets/img/cms_arrow.gif\" alt=\"Redigera\"/></a></div>");
