...
namespace Navipro.Infojet.Lib
{
    ...
    public class WebCartConfigLine
    {
        ...
        {
            DatabaseQuery databaseQuery = infojetContext.systemDatabase.prepare("SELECT [Entry No_], [Cart Entry No_], [Web Config Model No_], [Option Code], [Description], [Type], [Value], [Sort Order], [Config Description], [Value Description], [Visible] FROM [" + infojetContext.systemDatabase.getTableName("Web Cart Config Line") + "] WHERE [Entry No_] = @entryNo");
        ...
        public void save()
        {
            ...
            if (exists)
            {
                databaseQuery = infojetContext.systemDatabase.prepare("UPDATE [" + infojetContext.systemDatabase.getTableName("Web Cart Config Line") + "] SET [Cart Entry No_] = @cartEntryNo, [Web Config Model No_] = @webConfigModelNo, [Option Code] = @optionCode, [Description] = @description, [Type] = @type, [Value] = @value, [Sort Order] = @sortOrder, [Config Description] = @configDescription, [Value Description] = @valueDescription, [Visible] = @visible WHERE [Entry No_] = @entryNo");
                databaseQuery.addIntParameter("@entryNo", _entryNo);
            ...
            else
            {
                ...
                databaseQuery = infojetContext.systemDatabase.prepare("INSERT INTO [" + infojetContext.systemDatabase.getTableName("Web Cart Config Line") + "] ([Cart Entry No_], [Web Config Model No_], [Option Code], [Description], [Type], [Value], [Sort Order], [Config Description], [Value Description], [Visible]) VALUES (@cartEntryNo, @webConfigModelNo, @optionCode, @description, @type, @value, @sortOrder, @configDescription, @valueDescription, @visible)");
        ...
        {
            DatabaseQuery databaseQuery = infojetContext.systemDatabase.prepare("SELECT [Entry No_], [Cart Entry No_], [Web Config Model No_], [Option Code], [Description], [Type], [Value], [Sort Order], [Config Description], [Value Description], [Visible] FROM [" + infojetContext.systemDatabase.getTableName("Web Cart Config Line") + "] WHERE [Cart Entry No_] = @cartEntryNo");
