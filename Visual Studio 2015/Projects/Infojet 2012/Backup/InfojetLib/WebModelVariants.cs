...
namespace Navipro.Infojet.Lib
{
    public class WebModelVariants
    {
        ...
        public string getModelVariantNo(string itemNo)
        {
            string webModelNo = "";

            DatabaseQuery databaseQuery = infojetContext.systemDatabase.prepare("SELECT [Web Model No_] FROM [" + infojetContext.systemDatabase.getTableName("Web Model Variant") + "] WHERE [Item No_] = @itemNo");
            databaseQuery.addStringParameter("itemNo", itemNo, 20);

            SqlDataReader dataReader = databaseQuery.executeQuery();
            if (dataReader.Read())
            {
                webModelNo = dataReader.GetValue(0).ToString();
