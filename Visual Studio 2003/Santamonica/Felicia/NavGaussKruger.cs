...
namespace Navipro.SantaMonica.Felicia
{
	...
	/// </summary>
	public class NavGaussKruger
	{

		double axis = 0; // Semi-major axis of the ellipsoid.
		double flattening = 0; // Flattening of the ellipsoid.
		double central_meridian = 0; // Central meridian for the projection.
		//double lat_of_origin = 0; // Latitude of origin.
		double scale = 0; // Scale on central meridian.
		double false_northing = 0; // Offset for origo.
		double false_easting = 0; // Offset for origo.

		private static CultureInfo enUSCultureInfo =
			new CultureInfo("en-US");

		public NavGaussKruger(string projection)
		{
			//
		...
		private double[] calcRt90(double latitude, double longitude)
		{
		...
		private double[] calcWGS84(int x, int y) 
		{
		...
		private double math_sinh(double val) 
		{
			return 0.5 * (Math.Exp(val) - Math.Exp(-val));
		}
		private double math_cosh(double val) 
		{
			return 0.5 * (Math.Exp(val) + Math.Exp(-val));
		}
		private double math_atanh(double val) 
		{
		...
		private double GetLatLong(string LatorLong, bool isLong)
		{
			...
			if (LatorLong.IndexOf("'") > 0)
			{
				...
				deciLatLon += (Convert.ToDouble(sec.Insert(2, "."), enUSCultureInfo)
					) / 3600;
		...
		public int[] GetRT90(string latStr, string lonStr)
		{
			if (latStr != "" && lonStr != "")
			{
				// Parse NMEA-strings
				double lat = GetLatLong(latStr, false);
				double lon = GetLatLong(lonStr, true);

				Console.WriteLine("Latitude: "+lat);
				Console.WriteLine("Longitude: "+lon);

				// Calculate Projection on the RT90-grid
				double[] rt90Cord = this.calcRt90(lat, lon);

				// Return x,y vector
				int[] RT90Coordinates = 
					{
						Convert.ToInt32(Math.Round(rt90Cord[0])), 
						Convert.ToInt32(Math.Round(rt90Cord[1])) 
					};
