...
namespace Navipro.SantaMonica.Felicia
{
	...
	public class Orders : System.Windows.Forms.Form
	{
		...
		public void updateGrid()
		{
			...
			orderDataSet = dataOrderHeaders.getDataSet(0);
			readyDataSet = dataOrderHeaders.getDataSet(1);
			sentDataSet = dataOrderHeaders.getDataSet(2);

			int i = 0;
			while(i < orderDataSet.Tables[0].Rows.Count)
			{
				DataRow row = orderDataSet.Tables[0].Rows[i];
				row["no"] = smartDatabase.getSetup().agentId +"-"+ orderDataSet.Tables[0].Rows[i].ItemArray.GetValue(0).ToString();
				
				orderDataSet.Tables[0].Rows[i].AcceptChanges();
				i++;
			}
		...
		private void InitializeComponent()
		{
			...
			this.button12.Location = new System.Drawing.Point(56, 144);
			...
			this.statusLabel.Text = "Registrerade körorder";
			// 
			// orderGrid
			// 
			this.orderGrid.Location = new System.Drawing.Point(-3, 32);
			this.orderGrid.Size = new System.Drawing.Size(320, 104);
			this.orderGrid.TableStyles.Add(this.shipmentTable);
			// 
			// shipmentTable
			// 
			...
			this.shipmentTable.MappingName = "orderHeader";
			// 
			// noCol
			// 
			...
			this.label1.Text = "Registrerade körorder";
			// 
			// readyGrid
			// 
			...
			this.readyTable.MappingName = "orderHeader";
			// 
			// noReadyCol
			// 
			...
			this.label2.Text = "Registrerade körorder";
			// 
			// sentGrid
			// 
			...
			this.sentTable.MappingName = "orderHeader";
			// 
			// noSentCol
			// 
		...
		{
			if (orderGrid.CurrentRowIndex > -1)
			{
				Cursor.Current = Cursors.WaitCursor;
				Cursor.Show();

				DataOrderHeader dataOrderHeader = new DataOrderHeader(smartDatabase, int.Parse(orderDataSet.Tables[0].Rows[orderGrid.CurrentRowIndex].ItemArray.GetValue(0).ToString()));
				Order order = new Order(smartDatabase, dataOrderHeader, true);
		...
		private void button2_Click(object sender, System.EventArgs e)
		{
			if (readyGrid.CurrentRowIndex > -1)
			{
				...
				DataOrderHeader dataOrderHeader = new DataOrderHeader(smartDatabase, int.Parse(readyDataSet.Tables[0].Rows[readyGrid.CurrentRowIndex].ItemArray.GetValue(0).ToString()));
				Order order = new Order(smartDatabase, dataOrderHeader, true);
		...
		private void button4_Click(object sender, System.EventArgs e)
		{
			if (sentGrid.CurrentRowIndex > -1)
			{
				...
				DataOrderHeader dataOrderHeader = new DataOrderHeader(smartDatabase, int.Parse(sentDataSet.Tables[0].Rows[sentGrid.CurrentRowIndex].ItemArray.GetValue(0).ToString()));
				Order order = new Order(smartDatabase, dataOrderHeader, true);
		...
		{
			if ((this.orderGrid.DataSource != null) && (((DataTable)this.orderGrid.DataSource).Rows.Count > 0))
			{
				if (this.orderGrid.CurrentRowIndex > 0)
				{
					this.orderGrid.CurrentRowIndex = this.orderGrid.CurrentRowIndex - 1;
					this.orderGrid.Select(this.orderGrid.CurrentRowIndex);
				}
			}
		...
		private void button6_Click_1(object sender, System.EventArgs e)
		{
			if ((this.orderGrid.DataSource != null) && (((DataTable)this.orderGrid.DataSource).Rows.Count > 0))
			{
				if (this.orderGrid.CurrentRowIndex < (((DataTable)this.orderGrid.DataSource).Rows.Count -1))
				{
					this.orderGrid.CurrentRowIndex = this.orderGrid.CurrentRowIndex + 1;
					this.orderGrid.Select(this.orderGrid.CurrentRowIndex);
				}
